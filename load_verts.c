...
void load_vertices (AREA_FILE *area_file, const char *name, FILE *sheet, char *projection)
{
    int32_t          latd, lond, latm, lonm, i, j;
    float            lats, lons;
    char             lath, lonh, next_char, string[132]/* , id[20]*/;
    ...
    while (1)
    {
        ...
        if (!strncmp (string, "POINT=P-", 8))
        {
          ...
          for (j = 0 ; j < (int32_t) strlen (string) ; j++)
            {
                next_char = string[j];
                if (next_char == ';') break;
            }

  ...
  QWidget (parent, (Qt::WindowFlags) Qt::WA_StaticContents)
{
  ...
  if (map.textFont.isEmpty ()) map.textFont = "Charter";


  //  Setting the font weight to bold.  If you want to set the weight to ultralight (i.e. 0) use the setTextFont call.

  if (map.textWeight < 1 || map.textWeight > 99) map.textWeight = QFont::Bold;
...
void 
nvMap::setGridSpacing (double grid_inc_x, double grid_inc_y)
{
...
//!  Set the grid thickness (in pixels).

void 
nvMap::setGridThickness (int32_t thickness)
{
  map.grid_thickness = thickness;
...
void
nvMap::setZoomLevel (int32_t level)
{
...
void
nvMap::setZoomPercent (int32_t percent)
{
...
void
nvMap::setOverlapPercent (int32_t percent)
{
...
/*!  Zoom in to the specified bounds (see pfm_nvtypes.h for the NV_F64_XYMBR structure definition).
...
void
nvMap::zoomIn (NV_F64_XYMBR bounds, uint8_t setLevel)
{
...
void
nvMap::zoomInPercent (uint8_t setLevel)
{
  if (map.zoom_level < NVMAP_ZOOM_LEVELS)
    {
      double percentage = (double) map.zoom_percent / 100.0;

      double xsize = map.bounds[map.zoom_level].max_x - map.bounds[map.zoom_level].min_x;
      double ysize = map.bounds[map.zoom_level].max_y - map.bounds[map.zoom_level].min_y;


      //  Set a new zoom level if setLevel is NVTrue or if we only have the initial zoom level.

      int32_t old_level = map.zoom_level;
      if (!map.zoom_level || setLevel) map.zoom_level++;
...
void 
nvMap::setProjection (int32_t projection)
{
...
void 
nvMap::setCoastThickness (int32_t thickness)
{
...
void
nvMap::moveMap (double map_x, double map_y)
{
  double xsize = (map.bounds[map.zoom_level].max_x - map.bounds[map.zoom_level].min_x);
  double ysize = (map.bounds[map.zoom_level].max_y - map.bounds[map.zoom_level].min_y);
...
void
nvMap::moveMap (int32_t direction)
{
  double xsize = (map.bounds[map.zoom_level].max_x - map.bounds[map.zoom_level].min_x);
  double ysize = (map.bounds[map.zoom_level].max_y - map.bounds[map.zoom_level].min_y);

  double overlap_mult = 1.0 - ((double) map.overlap_percent / 100.0);
...
void 
nvMap::redrawMapWithCheck (uint8_t clear)
{
...
void
nvMap::redrawMapArea (uint8_t clear)
{
  //  Compute the center position

  double range = map.bounds[map.zoom_level].max_x - map.bounds[map.zoom_level].min_x;
  ...
  static int32_t save_movable_object_count = movable_object_count;
  movable_object_count = 0;
  ...
  if (clear)
    {
      ...
      if (map.grid_inc_x)
        {
          double min_x = map.bounds[map.zoom_level].min_x;
          double max_x = map.bounds[map.zoom_level].max_x;

          for (double x = min_x ; x < max_x ; x += map.grid_inc_x)
            {
              double map_x = (double) x;

              map_to_screen (1, &map_x, &map.bounds[map.zoom_level].min_y, &dum_map_z[0], &segx[0], &segy[0], &dum_z[0]);

              map_to_screen (1, &map_x, &map.bounds[map.zoom_level].max_y, &dum_map_z[0], &segx[1], &segy[1], &dum_z[0]);

              drawLine (segx[0], segy[0], segx[1], segy[1], map.grid_color, map.grid_thickness, NVFalse, Qt::SolidLine);
            }
        }
      ...
      if (map.grid_inc_y)
        {
          double min_y = map.bounds[map.zoom_level].min_y;
          double max_y = map.bounds[map.zoom_level].max_y;

          for (double y = min_y ; y < max_y ; y += map.grid_inc_y)
            {
              double map_y = (double) y;

              map_to_screen (1, &map.bounds[map.zoom_level].min_x, &map_y, &dum_map_z[0], &segx[0], &segy[0], &dum_z[0]);

              map_to_screen (1, &map.bounds[map.zoom_level].max_x, &map_y, &dum_map_z[0], &segx[1], &segy[1], &dum_z[0]);
...
nvMap::redrawMap (uint8_t clear)
{
  ...
  static char files[9][12] = {"coasts.dat", "rivers.dat", "bounds.dat", "wvsfull.dat", 
                                 "wvs250k.dat", "wvs1.dat", "wvs3.dat", "wvs12.dat", "wvs43.dat"};

  void wdbplt(char *file, double slatd, double nlatd, double wlond, 
              double elond, nvMap *map, QColor color, int32_t line_width);
  ...
  int32_t x0, y0, x1, y1;
  map_to_screen (1, &map.bounds[map.zoom_level].min_x, &map.bounds[map.zoom_level].max_y, &dum_map_z[0], &x0, &y0, &dum_z[0]);
  map_to_screen (1, &map.bounds[map.zoom_level].max_x, &map.bounds[map.zoom_level].min_y, &dum_map_z[0], &x1, &y1, &dum_z[0]);
  map.height = y1 - y0;
  map.width = x1 - x0;
  ...
  if (clear)
    {
      if (map.landmask)
        {
          int32_t start_lat = (int32_t) map.bounds[map.zoom_level].min_y - 1;
          int32_t start_lon = (int32_t) map.bounds[map.zoom_level].min_x - 1;
          int32_t end_lat = (int32_t) map.bounds[map.zoom_level].max_y + 1;
          int32_t end_lon = (int32_t) map.bounds[map.zoom_level].max_x + 1;
          double lat[2], lon[2];
          uint8_t *array = NULL;
          ...
          int32_t total = (end_lat - start_lat) * (end_lon - start_lon);
          ...
          for (int32_t j = start_lat ; j < end_lat ; j++)
            {
              for (int32_t k = start_lon ; k < end_lon ; k++)
                {
                  int32_t prog = (j - start_lat) * (end_lon - start_lon) + (k - start_lon);
                  ...
                  if (wsize > 2)
                    {
                      ...
                      for (int32_t m = 0 ; m < hsize ; m++)
                        {
                          lat[0] = (double) (j + 1) - (double) (m + 1) * hinc;
                          lat[1] = lat[0] + hinc;
                              ...
                              for (int32_t n = 0 ; n < wsize ; n++)
                                {
                                  if (array[m * wsize + n])
                                    {
                                      lon[0] = (double) k + (double) (n + 1) * winc;
                                      lon[1] = lon[0] + winc;
      ...
      if (map.coasts)
        {
          float xsize = map.bounds[map.zoom_level].max_x - map.bounds[map.zoom_level].min_x;
          float ysize = map.bounds[map.zoom_level].max_y - map.bounds[map.zoom_level].min_y;


          //  This is the 1:50,000 scale coastline.  It does not have data north of 60N or south of 57S.

          if (map.coasts == NVMAP_NGA_50K_COAST || (map.coasts == NVMAP_AUTO_COAST && check_coast (COAST_50K) && xsize < 5.0 && ysize < 5.0 && 
                                                    map.bounds[map.zoom_level].max_y < 60.0 && map.bounds[map.zoom_level].min_y > -56.0))
            {
              map.coasts_used = NVMAP_NGA_50K_COAST;

              int32_t slat = (int32_t) (map.bounds[map.zoom_level].min_y + 90.0) - 90;
              int32_t nlat = (int32_t) (map.bounds[map.zoom_level].max_y + 90.0) - 89;
              int32_t wlon = (int32_t) (map.bounds[map.zoom_level].min_x + 180.0) - 180;
              int32_t elon = (int32_t) (map.bounds[map.zoom_level].max_x + 180.0) - 179;
  ...
  if (clear)
    {
      ...
      if (map.grid_inc_x)
        {
          double min_x = map.bounds[map.zoom_level].min_x * 60.0;
          double max_x = map.bounds[map.zoom_level].max_x * 60.0;

          for (double x = min_x ; x < max_x ; x += map.grid_inc_x)
            {
              double map_x = (double) x / 60.0;
      ...
      if (map.grid_inc_y)
        {
          double min_y = map.bounds[map.zoom_level].min_y * 60.0;
          double max_y = map.bounds[map.zoom_level].max_y * 60.0;

          for (double y = min_y ; y < max_y ; y += map.grid_inc_y)
            {
              double map_y = (double) y / 60.0;
...
int32_t nvMap::create_movable_object (NVMAP_MOVABLE_OBJECT mo)
{
  for (int32_t i = 0 ; i < MAX_MOVABLE_OBJECTS ; i++)
    {
...
void nvMap::discard_movable_object (int32_t *num)
{
      ...
      if (ot[i] == &movable_object[*num])
        {
          ...
          for (int32_t j = i ; j < movable_object_count - 1 ; j++) ot[j] = ot[j + 1];
...
void 
nvMap::anchorRubberbandPolygon (int32_t *num, double map_x, double map_y, QColor color, int32_t line_width, uint8_t close,
...
void 
nvMap::anchorRubberbandPolygon (int32_t *num, int32_t x, int32_t y, QColor color, int32_t line_width, uint8_t close,
                                Qt::PenStyle penStyle)
{
...
void 
nvMap::dragRubberbandPolygon (int32_t num, double map_x, double map_y)
{
  int32_t x, y;
...
void 
nvMap::dragRubberbandPolygon (int32_t num, int32_t x, int32_t y)
{
...
void 
nvMap::getRubberbandPolygonCurrentAnchor (int32_t num, double *map_x, double *map_y)
{
  if (num < 0) return;
...
void 
nvMap::vertexRubberbandPolygon (int32_t num, double map_x, double map_y)
{
  int32_t x, y;
...
void 
nvMap::vertexRubberbandPolygon (int32_t num, int32_t x, int32_t y)
{
...
void 
nvMap::getRubberbandPolygon (int32_t num, int32_t *count, int32_t **px, int32_t **py, double **mx, double **my)
{
  if (num < 0) return;
...
/*!  Close a rubberband polygon in map space adding the input point (map_x, map_y).  Returns the
     ...
     px, py, mx, and my should be declared in your calling function as int32_t *px and 
     double *mx.  They should be passed to this function as &px.  Remember to "discard"
     the polygon to free the associated allocated memory (returned as px, py, mx, my).  */

void 
nvMap::closeRubberbandPolygon (int32_t num, double map_x, double map_y, int32_t *count, int32_t **px, 
...
void 
nvMap::closeRubberbandPolygon (int32_t num, int32_t x, int32_t y, int32_t *count, int32_t **px, int32_t **py, 
                               double **mx, double **my)
{
...
uint8_t
nvMap::rubberbandPolygonIsActive (int32_t num)
{
...
void 
nvMap::discardRubberbandPolygon (int32_t *num)
{
...
void 
nvMap::anchorRubberbandRectangle (int32_t *num, double map_x, double map_y, QColor color, int32_t line_width,
                                  Qt::PenStyle penStyle)
{
  int32_t x, y;
...
void 
nvMap::anchorRubberbandRectangle (int32_t *num, int32_t x, int32_t y, QColor color, int32_t line_width, Qt::PenStyle penStyle)
{
  NVMAP_MOVABLE_OBJECT mo;
...
void 
nvMap::dragRubberbandRectangle (int32_t num, double map_x, double map_y)
{
  int32_t x, y;
...
void 
nvMap::dragRubberbandRectangle (int32_t num, int32_t x, int32_t y)
{
...
void 
nvMap::getRubberbandRectangle (int32_t num, int32_t **px, int32_t **py, double **mx, double **my)
{
  if (num < 0) return;
...
/*!  Close a rubberband rectangle in map space.  The ending corner is specified by the point
     ...
     in your calling function as int32_t *px and double *mx.  They should be passed to
     this function as &px.  */

void 
nvMap::closeRubberbandRectangle (int32_t num, double map_x, double map_y, int32_t **px, int32_t **py, 
                                 double **mx, double **my)
{
  int32_t x, y;
...
/*!  Close a rubberband rectangle in screen space.  The ending corner is specified by the point
     ...
     function as int32_t *px and double *mx.  They should be passed to this function as &px.  */

void 
nvMap::closeRubberbandRectangle (int32_t num, int32_t x, int32_t y, int32_t **px, int32_t **py, double **mx, 
                                 double **my)
{
...
uint8_t 
nvMap::rubberbandRectangleIsActive (int32_t num)
{
  if (num < 0) return (NVFalse);
...
void 
nvMap::getRubberbandRectangleAnchor (int32_t num, double *map_x, double *map_y)
{
  if (num < 0) return;
...
void 
nvMap::discardRubberbandRectangle (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void 
nvMap::anchorRubberbandLine (int32_t *num, double map_x, double map_y, QColor color, int32_t line_width, Qt::PenStyle penStyle)
{
  int32_t x, y;
...
void 
nvMap::anchorRubberbandLine (int32_t *num, int32_t x, int32_t y, QColor color, int32_t line_width, Qt::PenStyle penStyle)
{
  NVMAP_MOVABLE_OBJECT mo;
...
void 
nvMap::dragRubberbandLine (int32_t num, double map_x, double map_y)
{
  int32_t x, y;
...
void 
nvMap::dragRubberbandLine (int32_t num, int32_t x, int32_t y)
{
...
uint8_t 
nvMap::rubberbandLineIsActive (int32_t num)
{
...
void 
nvMap::discardRubberbandLine (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void
nvMap::setMovingRectangle (int32_t *num, double map_x0, double map_y0, double map_x1, double map_y1, 
...
void
nvMap::setMovingRectangle (int32_t *num, int32_t x0, int32_t y0, int32_t x1, int32_t y1, QColor color,
...
void 
nvMap::getMovingRectangle (int32_t num, int32_t **px, int32_t **py, double **mx, double **my)
{
...
void 
nvMap::closeMovingRectangle (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void
nvMap::setMovingPolygon (int32_t *num, int32_t count, double map_x[], double map_y[], QColor color,
                         int32_t line_width, uint8_t filled, Qt::PenStyle penStyle)
{
  int32_t *x = (int32_t *) calloc (count + 1, sizeof (int32_t));
  int32_t *y = (int32_t *) calloc (count + 1, sizeof (int32_t));
  int32_t *z = (int32_t *) calloc (count + 1, sizeof (int32_t));
  double *dz = (double *) calloc (count + 1, sizeof (double));
...
void
nvMap::setMovingPolygon (int32_t *num, int32_t count, int32_t *x, int32_t *y, QColor color, int32_t line_width,
...
void 
nvMap::getMovingPolygon (int32_t num, int32_t *count, int32_t **px, int32_t **py, double **mx, double **my)
{
  getRubberbandPolygon (num, count, px, py, mx, my);
}
...
void 
nvMap::closeMovingPolygon (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void
nvMap::setMovingPolyline (int32_t *num, int32_t count, double map_x[], double map_y[], QColor color,
...
void
nvMap::setMovingPolyline (int32_t *num, int32_t count, int32_t *x, int32_t *y, QColor color, int32_t line_width,
                          Qt::PenStyle penStyle)
{
  if (count > NVMAP_POLYGON_POINTS)
    {
...
void 
nvMap::getMovingPolyline (int32_t num, int32_t *count, int32_t **px, int32_t **py, double **mx, double **my)
{
  getRubberbandPolygon (num, count, px, py, mx, my);
}
...
void 
nvMap::closeMovingPolyline (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void
nvMap::setMovingCircle (int32_t *num, double map_x, double map_y, int32_t d, float start_angle, 
...
void
nvMap::setMovingCircle (int32_t *num, double map_x, double map_y, double end_map_x, double end_map_y, 
                        float start_angle, float span_angle, int32_t line_width, QColor color,
                        uint8_t filled, Qt::PenStyle penStyle)
{
  int32_t x0, y0, x1, y1, x2, y2, d;
  ...
  d = NINT (sqrt (double ((y2 * y2) + (x2 * x2))) * 2.0);
...
void
nvMap::setMovingCircle (int32_t *num, int32_t x, int32_t y, int32_t d, float start_angle, float span_angle, 
...
void
nvMap::getMovingCircle (int32_t num, double *map_x, double *map_y, int32_t *x, int32_t *y, int32_t *d)
{
...
void 
nvMap::closeMovingCircle (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void 
nvMap::setMovingPath (int32_t *num, QPainterPath path, double map_center_x, double map_center_y,
...
void 
nvMap::setMovingPath (int32_t *num, QPainterPath path, int32_t center_x, int32_t center_y, int32_t line_width,
...
void 
nvMap::closeMovingPath (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void 
nvMap::setMovingText (int32_t *num, QString text, double map_center_x, double map_center_y,
                      QColor color, double angle, int32_t size)
{
  int32_t center_x, center_y;
...
void 
nvMap::setMovingText (int32_t *num, QString text, int32_t center_x, int32_t center_y, QColor color, double angle, int32_t size)
{
  //  If the object number is less than zero it has been discarded and, thus, needs to be created.
...
void 
nvMap::closeMovingText (int32_t *num)
{
  if (*num < 0 || !movable_object[*num].active) return;
...
void
nvMap::drawText (QString string, double map_x, double map_y, double angle, int32_t size, QColor color, uint8_t flush)
{
...
void
nvMap::drawTextClear (QString string, double map_x, double map_y, double angle, int32_t size, QColor color,
...
void
nvMap::drawText (QString string, int32_t x, int32_t y, double angle, int32_t size, QColor color, uint8_t flush)
{
...
void
nvMap::drawTextClear (QString string, int32_t x, int32_t y, double angle, int32_t size, QColor color, uint8_t clear, uint8_t flush)
{
  QFont font (map.textFont, size);
  font = QFont (font, painter.device ());
  font.setWeight (map.textWeight);
...
void
nvMap::drawText (QString string, double map_x, double map_y, QColor color, uint8_t flush)
{
  int32_t x, y;
...
void
nvMap::drawText (QString string, int32_t x, int32_t y, QColor color, uint8_t flush)
{
  drawText (string, x, y, 90.0, 12, color, flush);
}
...
/*!  Sets the font and font weight to be used for all subsequent text strings (default font is "Charter", default weight is QFont::Bold).
     The weights can be anything from 0 to 99 but the normal values are:
     - QFont::Light    (25)
     - QFont::Normal   (50)
     - QFont::DemiBold (63)
     - QFont::Bold     (75)
...
void
nvMap::setTextFont (QString string, int32_t weight)
{
  map.textFont = string;
  map.textWeight = weight;
}
...
void
nvMap::drawLine (double map_x0, double map_y0, double map_x1, double map_y1,
                 QColor color, int32_t line_width, uint8_t flush, Qt::PenStyle penStyle)
{
  int32_t x0, y0, x1, y1;
...
void
nvMap::drawLine (int32_t x0, int32_t y0, int32_t x1, int32_t y1, QColor color, int32_t line_width, uint8_t flush,
                 Qt::PenStyle penStyle)
{
  QPen pen;
  pen.setColor (color);
...
void
nvMap::drawCircle (double map_x, double map_y, int32_t d, float start_angle, float span_angle,
...
void
nvMap::drawCircle (double map_x, double map_y, double end_map_x, double end_map_y, float start_angle,
...
void
nvMap::drawCircle (int32_t x, int32_t y, int32_t d, float start_angle, float span_angle,
                   int32_t line_width, QColor color, Qt::PenStyle penStyle, uint8_t flush)
{
  //  Center the circle.
...
void
nvMap::fillCircle (double map_x, double map_y, int32_t d, float start_angle, 
...
void
nvMap::fillCircle (double map_x, double map_y, double end_map_x, double end_map_y, 
...
void
nvMap::fillCircle (int32_t x, int32_t y, int32_t d, float start_angle, float span_angle, QColor color, uint8_t flush)
{
  //  Center the circle.
...
void 
nvMap::fillRectangle (double map_x, double map_y, int32_t w, int32_t h, QColor color, uint8_t flush)
{
  int32_t x, y;
...
void 
nvMap::fillRectangle (double map_x, double map_y, double end_map_x, double end_map_y, QColor color,
                    uint8_t flush)
{
  int32_t x, y, w, h;
...
void
nvMap::fillRectangle (int32_t x, int32_t y, int32_t w, int32_t h, QColor color, uint8_t flush)
{
...
void
nvMap::drawRectangle (double map_x0, double map_y0, double map_x1, double map_y1,
...
void
nvMap::drawRectangle (double map_x0, double map_y0, int32_t w, int32_t h,
                      QColor color, int32_t line_width, Qt::PenStyle penStyle, uint8_t flush)
{
  int32_t x0, y0;
...
void
nvMap::drawRectangle (int32_t x0, int32_t y0, int32_t x1, int32_t y1, QColor color, int32_t line_width,
...
void
nvMap::fillPolygon (int32_t count, double map_x[], double map_y[], QColor color, uint8_t flush)
{
  int32_t *x = (int32_t *) calloc (count, sizeof (int32_t));
  int32_t *y = (int32_t *) calloc (count, sizeof (int32_t));
  int32_t *z = (int32_t *) calloc (count, sizeof (int32_t));
...
void
nvMap::fillPolygon (int32_t count, NV_F64_COORD2 map_xy[], QColor color, uint8_t flush)
{
  double *x = (double *) calloc (count, sizeof (double));
  double *y = (double *) calloc (count, sizeof (double));
...
void
nvMap::fillPolygon (int32_t count, int32_t x[], int32_t y[], QColor color, uint8_t flush)
{
  //  Don't outline the rectangle
  ...
  for (int32_t i = 0 ; i < count ; i++) poly.setPoint (i, x[i], y[i]);
...
void
nvMap::fillPolygon (int32_t count, NV_I32_COORD2 xy[], QColor color, uint8_t flush)
{
  int32_t *x = (int32_t *) calloc (count, sizeof (int32_t));
  int32_t *y = (int32_t *) calloc (count, sizeof (int32_t));
...
void
nvMap::drawPolygon (int32_t count, double map_x[], double map_y[], QColor color, int32_t line_width,
...
void
nvMap::drawPolygon (int32_t count, NV_F64_COORD2 map_xy[], QColor color, int32_t line_width,
                    uint8_t close, Qt::PenStyle penStyle, uint8_t flush)
{
  double *x = (double *) calloc (count, sizeof (double));
  double *y = (double *) calloc (count, sizeof (double));
...
void
nvMap::drawPolygon (int32_t count, int32_t x[], int32_t y[], QColor color, int32_t line_width, uint8_t close, 
                    Qt::PenStyle penStyle, uint8_t flush)
{
  //  Make sure we close the polygon.

  int32_t *new_x = (int32_t *) calloc (count + 1, sizeof (int32_t));
  int32_t *new_y = (int32_t *) calloc (count + 1, sizeof (int32_t));
  ...
  for (int32_t i = 0 ; i < count ; i++) drawLine (new_x[i], new_y[i], new_x[i + 1], new_y[i + 1], color, line_width,
                                                   flush, penStyle);
...
void
nvMap::drawPolygon (int32_t count, NV_I32_COORD2 xy[], QColor color, int32_t line_width, uint8_t close, 
                    Qt::PenStyle penStyle, uint8_t flush)
{
  int32_t *x = (int32_t *) calloc (count, sizeof (int32_t));
  int32_t *y = (int32_t *) calloc (count, sizeof (int32_t));
...
void 
nvMap::drawPath (QPainterPath path, double map_center_x, double map_center_y, int32_t line_width, QColor color,
...
void 
nvMap::drawPath (QPainterPath path, int32_t center_x, int32_t center_y, int32_t line_width, QColor color,
                 QBrush brush, uint8_t filled, Qt::PenStyle penStyle, uint8_t flush)
{
  ...
  int32_t x_offset = center_x - (int32_t) (r2.width () / 2.0);
  int32_t y_offset = center_y - (int32_t) (r2.height () / 2.0);
...
void 
nvMap::drawPixmap (int32_t x, int32_t y, QPixmap *src, int32_t src_x, int32_t src_y, int32_t src_w, int32_t src_h, uint8_t flush)
{
...
QPixmap 
nvMap::grabPixmap (int32_t x, int32_t y, int32_t w, int32_t h)
{
...
nvMap::grabPixmap ()
{
  int32_t start = map.draw_height - map.height - map.border;
...
/*!  Convert map coordinates to screen coordinates.  Z is usually ignored.  This version returns
     double pixels so you don't have to convert them going into some other functions.  */

void 
nvMap::map_to_screen (int32_t num_points, double map_x[], double map_y[], 
                      double map_z[], double pix_x[], double pix_y[], double pix_z[])
{
  int32_t *x = (int32_t *) malloc (num_points * sizeof (int32_t));
  int32_t *y = (int32_t *) malloc (num_points * sizeof (int32_t));
  int32_t *z = (int32_t *) malloc (num_points * sizeof (int32_t));
...
{
  double *dx = (double *) calloc (num_points, sizeof (double));
  double *dy = (double *) calloc (num_points, sizeof (double));
  double *dz = (double *) calloc (num_points, sizeof (double));
  ...
  if (!last_map_called || map.projection == NO_PROJECTION)
      ...
      if (invertY)
        {
          for (int32_t i = 0 ; i < num_points ; i++)
            {
              dx[i] = (double) map.draw_width * ((map_x[i] - map.bounds[map.zoom_level].min_x) /
                                                     (map.bounds[map.zoom_level].max_x -
                                                      map.bounds[map.zoom_level].min_x)); 
              dy[i] = (double) map.draw_height * ((map_y[i] - map.bounds[map.zoom_level].max_y) /
          ...
          for (int32_t i = 0 ; i < num_points ; i++)
            {
              dx[i] = (double) map.draw_width * ((map_x[i] - map.bounds[map.zoom_level].min_x) /
                                                     (map.bounds[map.zoom_level].max_x - 
                                                      map.bounds[map.zoom_level].min_x));
              dy[i] = (double) map.draw_height * ((map.bounds[map.zoom_level].max_y - map_y[i]) /
                                                      (map.bounds[map.zoom_level].max_y -
                                                       map.bounds[map.zoom_level].min_y));
            }
        }
  ...
  else
    {
      ...
      double *mx = (double *) calloc (num_points, sizeof (double));
...
void 
nvMap::screen_to_map (int32_t num_points, double map_x[], double map_y[], double map_z[], int32_t pix_x[], int32_t pix_y[], int32_t pix_z[])
{
  double *dx = (double *) calloc (num_points, sizeof (double));
  double *dy = (double *) calloc (num_points, sizeof (double));
  double *dz = (double *) calloc (num_points, sizeof (double));
...
void 
nvMap::screen_to_map (int32_t num_points, double map_x[], double map_y[], double map_z[], double pix_x[], double pix_y[], double pix_z[])
{
  //  Unprojected data or last_map_called == 0 (called redrawMapArea).
          ...
          for (int32_t i = 0 ; i < num_points ; i++)
            {
              ...
              map_y[i] = map.bounds[map.zoom_level].min_y +
                ...
                (((double) map.draw_height - pix_y[i]) / (double) map.draw_height);
            }
        }
    }
  else
...
uint8_t 
nvMap::checkBounds (double map_x, double map_y)
{
  double mx = map_x, my = map_y;
...
void 
nvMap::getMBR (double *map_x, double *map_y, int32_t num_points, NV_F64_XYMBR *mbr)
{
  double *mx = (double *) calloc (num_points, sizeof (double));
...
void 
nvMap::setZeroToThreeSixty (uint8_t enable)
{
  zero_to_360 = enable;
}
...
void 
nvMap::setNoBoundsLimit (uint8_t enable)
{
...
int32_t 
nvMap::clipLine (double map_x1, double map_y1, double map_x2, double map_y2, 
                 double *ret_x1, double *ret_y1, double *ret_x2, double *ret_y2)
{
  double x[2], y[2], mod_x1 = map_x1, mod_x2 = map_x2, mod_y1 = map_y1, mod_y2 = map_y2;
  int32_t hits, retval;
...
void 
nvMap::boundsCheck (double *map_x, double *map_y)
{
  double mx = *map_x;
...
void 
nvMap::dummyBuffer (int32_t width, int32_t height, uint8_t start)
{
  static int32_t ow = 0, oh = 0;
  ...
  if (start)
    {
      ...
      buffer.fill (map.background_color);
...
nvMap::resizeEvent (QResizeEvent *e)
{
  ...
  int32_t w = e->size ().width ();
  int32_t h = e->size ().height ();
  ...
  if (movable_object_count)
    {
      ...
      for (int32_t k = 0 ; k < movable_object_count ; k++)
        {
          int32_t inc = ot[k]->line_width / 2 + 1;
          int32_t inc2 = inc * 2;
          int32_t newx, newy;
          QPen pen;
          QRect r;
          ...
          switch (ot[k]->type)
            ...
            case NVMAP_MOVE_POLYGON:
              ...
              for (int32_t i = 0 ; i < ot[k]->polygon_count ; i++) 
                {
            ...
            case NVMAP_MOVE_PATH:
              ...
              screen_painter.save ();
              screen_painter.translate (ot[k]->x - (int32_t) (r2.width () / 2.0),
                                        ot[k]->y - (int32_t) (r2.height () / 2.0));
              ...
              ot[k]->prev_rect.setCoords ((int32_t) r2.normalized().x() - inc, 
                                          (int32_t) r2.normalized().y() - inc,
                                          (int32_t) r2.normalized().x() +
                                          (int32_t) r2.normalized().width() + inc2,
                                          (int32_t) r2.normalized().y() +
                                          (int32_t) r2.normalized().height() + inc2);
...
void 
nvMap::init_proj (int32_t x, int32_t y, int32_t width, int32_t height, int32_t border, double min_x, 
                double min_y, double max_x, double max_y)
{
  ...
  proj = new rproj (map.projection, 0.0, 0.0, (double) (x + border), (double) ((x + width) - border), 
                    (double) ((y + height) - border), (double) (y + border), min_y, max_y, min_x, max_x, 
                    &dum1, &dum2, &dum3);
}
...
int32_t 
nvMap::lineIntersection (double x1, double y1, double x2, double y2,

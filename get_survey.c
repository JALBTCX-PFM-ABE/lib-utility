...
/*ARGSUSED*/
void fgetsrv_ (char *directory, char *areaname, char *archive, int32_t dirlen __attribute__ ((unused)),
               int32_t arealen __attribute__ ((unused)), int32_t arclen __attribute__ ((unused)))
{
  char       srv_file[132], string[132], dirs[21][132], areas[21][50], archives[21][10];
  FILE    *srv;
  int32_t      j, selection, manual, count;

  sprintf (srv_file, "%s%1csurveys", getenv ("GLOBAL_DIR"), (char) SEPARATOR);
      ...
      while (1)
        {
          ...
          if (string[0] != ';')
            {
              for (j = (int32_t) strlen (string) ; j >= 0 ; j--)
                {
                  if (string[j] != ' ') break;
                  string[j] = 0;
                }
...
/*ARGSUSED*/
void fgetsrv (char *directory, char *areaname, char *archive, int32_t dirlen,
              int32_t arealen, int32_t arclen)
{
...
void get_survey (char *directory, char *areaname, char *archive)
{
  int32_t      dirlen, arealen, arclen;

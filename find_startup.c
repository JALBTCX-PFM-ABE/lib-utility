...
FILE *find_startup (const char *file_name)
{
  char             path[4096], file[512], tmp[512];
  FILE             *fp;
  ...
  if (getenv ("HOME") != NULL)
    {
      sprintf (file, "%s%1c%s", getenv ("HOME"), SEPARATOR, file_name);
  ...
  if (getenv ("PATH") != NULL)
    {
      ...
      for (i = 0 ; i < (int32_t) strlen (path) ; i++)
        {

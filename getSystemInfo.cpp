...
#else
#   ifdef __MINGW32__                //  On MinGW32 we have to set the minimum
#       define _WIN32_WINNT 0x0501   //  Windows version to 0x0501 (Windows XP)
#       define WINVER 0x0501         //  prior to including the windows.h file or
#   endif                            //  the MEMORYSTATUSEX function is undefined.
#   include <windows.h>
#endif
...
/*!
  ...
  - Acknowledgments:    Most of this code was borrowed from a post by Stuart
                        Nixon on the qt-interest forum and some stuff from
                        Andrew T on stackoverflow.com.

****************************************************************************/

void getSystemInfo (uint32_t *cpu_count, uint64_t *memory)
{
  *cpu_count = 1;
  *memory = -1;
  ...
  *cpu_count = sysconf (_SC_NPROCESSORS_ONLN);
  ...
  int32_t pages = sysconf (_SC_AVPHYS_PAGES);
  int32_t page_size = sysconf (_SC_PAGE_SIZE);
  ...
  *memory = (uint64_t) pages * (uint64_t) page_size;
          ...
          while (fgets (string, sizeof (string), fp) != NULL)
            {
              if (!strncmp (string, "MemFree:", 8))
                {
                  sscanf (string, "MemFree: %"PRId64, memory);
  ...
  uint8_t ret = GlobalMemoryStatusEx (&status);
  if (ret) *memory = status.ullAvailPhys;

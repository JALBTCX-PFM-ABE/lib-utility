...
static void interpolate (int32_t num_interp, int32_t segment, double *coef_x, double *coef_y, double *contour_x, double *contour_y,
                         int32_t interp_section)
{
  double       increment, t, t2, t3;
  int32_t      i, index;


  increment = 1.0 / (float) num_interp;
...
void smooth_contour (int32_t num_interp, int32_t *num_pts, float *contour_x, float *contour_y)
{
  double *x64;
  double *y64;

  if ((x64 = (double *) malloc (*num_pts * sizeof (double))) == NULL)
    {
      perror ("Allocating x64 in smooth_contour.cpp");
      exit (-1);
    }

  if ((y64 = (double *) malloc (*num_pts * sizeof (double))) == NULL)
    {
  ...
  for (int32_t i = 0 ; i < *num_pts ; i++)
    {
      x64[i] = (double) contour_x[i];
      y64[i] = (double) contour_y[i];
    }
  ...
  for (int32_t i = 0 ; i < *num_pts ; i++)
    {
      contour_x[i] = (float) x64[i];
      contour_y[i] = (float) y64[i];
    }
...
void smooth_contour (int32_t num_interp, int32_t *num_pts, double *contour_x, double *contour_y)
{
  double      *raw_x, *raw_y;
  double      x[4], y[4];
  ...
  void b_spline(double *, double *, double *, double *);


  // allocate memory and make copies of the original contour data

  if ((raw_x = (double *) malloc ((*num_pts + 2) * sizeof (double))) == NULL)
    {
  ...
  if ((raw_y = (double *) malloc ((*num_pts + 2) * sizeof (double))) == NULL)
    {
      perror ("Allocating raw_y in smooth_contour.cpp");
      exit (-1);
    }

  memcpy (raw_x + 1, contour_x, *num_pts * sizeof (double));
  memcpy (raw_y + 1, contour_y, *num_pts * sizeof (double));

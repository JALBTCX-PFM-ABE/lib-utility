...
int32_t get_coords (char *lat_hemi, int32_t *lat_deg, int32_t *lat_min, float *lat_sec,
char *lon_hemi, int32_t *lon_deg, int32_t *lon_min, float *lon_sec)
{
    ...
    for (j = 0 ; j < 2 ; j++)
    {
        ...
        if (j)
        {
            ...
            *lon_sec = (float)((fmin - *lon_min) * 60.0 + 0.00001);
        }
        else
        {
            if (sign)
            ...
            *lat_deg = (int32_t)fdeg;
            fmin = (int32_t)((fdeg - *lat_deg) * 60.0);
            *lat_min = (int32_t)(fmin + 0.00001);
            *lat_sec = (float)((fmin - *lat_min) * 60.0 + 0.00001);
        }
    }

...
uint8_t carter (double lat, double lon, double nominal, double *depth, int32_t *cor_area)
{
  int32_t i, lat_indx, lon_use;
  uint8_t found;
  double cordepth_high = 0, cordepth_low = 0, uncdepth_low = 0, offset;
  ...
  *cor_area = 99999;
  ...
  lat_indx = 89 - (int32_t) floor (lat);   /* 89-+89=0, ..., 89--91=180 */
  lon_use = (int32_t) floor (lon);
  ...
  for (i = 0 ; !found && i < ncor[*cor_area - 1] - 1 ; i++) 
    {
      if ((nominal >= uncdepth[i]) && (nominal < uncdepth[i + 1]))
        {
          cordepth_high = (double) (cor[*cor_area - 1][i + 1]);
          cordepth_low = (double) (cor[*cor_area - 1][i]);
          uncdepth_low = (double) ( uncdepth[i] );
          found = NVTrue;
        }
    }
  if (!found)
  ...
  offset = (double) nominal - uncdepth_low;
...
uint8_t rev_carter (double lat, double lon, double tru, double *depth, int32_t *cor_area)
{
  int32_t i, lat_indx, lon_use;
  uint8_t found;
  double uncdepth_high = 0, uncdepth_low = 0, cordepth_low = 0, offset;
  ...
  *cor_area = 99999;
  ...
  for (i = 0 ; !found && i < ncor[*cor_area - 1] - 1 ; i++) 
    {
      if ((tru >= cor[*cor_area - 1][i]) && (tru < cor[*cor_area - 1][i + 1]))
        {
          uncdepth_high = (double) (uncdepth[i + 1]);
          uncdepth_low = (double) (uncdepth[i]);
          cordepth_low = (double) (cor[*cor_area - 1][i]);
          found = NVTrue;
        }
  ...
  offset = (double) tru - cordepth_low;

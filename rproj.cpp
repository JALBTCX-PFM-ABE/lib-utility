...
rproj::rproj (int32_t projection, double dum1, double dum2, double min_x_pix,
...
double rproj::dm (double rdm)
{
    ...
    deg = (int32_t) (rdm / 100.0);

    return ((double) (rdm - deg * 40.0));
}




double rproj::udm (double rmin)
{
    int32_t     deg, min;

    /*  Inverse of dm.                                                  */

    min = (int32_t) (fmod ((double) rmin, 60.0) + 0.5);
    deg = (int32_t) rmin / 60;
    return ((double) (deg * 100 + min));
}
...
*   Inputs:             lat                 -   latitude                    *
*                                                                           *
*   Outputs:            double              -   meridional parts            *
*                                                                           *
*   Method:             Bowditch                                            *
*
</pre>                                                                           *
...
double rproj::latmp (double lat)
{
    double      y, t, ym, answer;
...
double rproj::mplat (double mp)
{
    double      amp, blat, bl, bmp, answer;
...
void rproj::project (int32_t n, int32_t unit, double rlonar[], double rlatar[], 
                     double rdatar[], double rx[], double ry[], double rz[])
{
    double          a, asq, b, g, q, r, rcolat, rlat, rlon, theta, x, y,
                    p, psq;
    int32_t         i;
    ...
    for (i = 0 ; i < n ; i++)
      {
        switch (unit)
          ...
          case RPROJ_D_M:
            rlat = dm ((double) rlatar[i]) * RADMIN;
            rlon = dm ((double) rlonar[i]) * RADMIN;
            break;
...
void rproj::unproject (int32_t n, int32_t unit, double rlonar[], double rlatar[], 
                       double rdatar[], double rx[], double ry[], double rz[])
{
    double          a1, c, c1, c2, c3, d, r, rcolat, rlat, rlon, theta,
    ...
    for (i = 0 ; i < n ; i++)
    {
        ...
        else
        {
            ...
            switch (unit)
              ...
              case RPROJ_D_M:
                rlatar[i] = udm ((double) rlat * MINRAD);
                rlonar[i] = udm ((double) rlon * MINRAD);
                break;
...
void rproj::pmaplm (double rlats, double rlatn, double rlonw, double rlone, 
                    double *sf, double *xsize, double *ysize)
{
    double          dlat, dlon, xmax, xmin, ymax, ymin;
    double          rlat, rlon, rdat, rx, ry, rz;
    int32_t         i, j, lck;
...
void rproj::pdrnge (double dum1, double dum2, double dum3, double dum4)
{
...
void rproj::pscale (double dummy)
{

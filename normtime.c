...
void unnormtime (double time, int32_t *year, int32_t *jday, int32_t *hour, int32_t *minute, float *second)
{
  ...
  *second = (float) time_ptr->tm_sec + (float) (psec);
}
...
double normtime (int32_t year, int32_t jday, int32_t hour, int32_t min, float sec)
{
  struct tm                    tm;
  static int32_t               tz_set = 0;
  time_t                       tv_sec;
  ...
  return ((double) tv_sec + (double) tv_nsec / 1000000000.0L);
}

...
/*!
   ...
   - Purpose :       We want to be able to register ABE applications to groups
                     and keep track of their shared memory IDs so that we can
                     start applications (other than by shelling) and still
                     share some positional information.  Each process will
                     have a group number and a member ID.  The member ID will
                     always be the calling application's PID.  If you look at
                     ABE_register.hpp you'll see how the memory block is
...
registerABE::registerABE (QWidget *parent, const char *name, int32_t key, const char *file, QSharedMemory **abeRegister,
  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  *abe_register = (ABE_REGISTER *) (*abeRegister)->data ();
  ...
  if (!attached)
    {
      ...
      for (int32_t i = 0 ; i < MAX_ABE_GROUPS ; i++)
	{
  ...
  for (int32_t i = 0 ; i < max_members * 2 ; i += 2)
    {

...
/*!
  ...
  - Synopsis        check_target_schema (target_path);
                    - char *target_path - target file name
...
uint8_t check_target_schema (const char *target_path)
{
  FILE *tfp, *cfp, *ofp;
  char string[8192], *ptr, fname[512], tmp_fname[512], blank = ' ';
  uint8_t eos, ret;
  int32_t i = 0, loc, size;

...
/*ARGSUSED*/
void fgetarea_ (char *area, int32_t *count, char *dirname, int32_t arealen __attribute__ ((unused)), 
                int32_t dirlen __attribute__ ((unused)))
{
#ifndef NVWIN3X
    struct dirent   *dp;
    DIR             *directory;
	...
	while ( validFile )
	{
		if (strlen ( (const char *)findData.cFileName) == 3)
		{
			strcpy (area + *count * 80, (const char *)findData.cFileName);
			(*count)++;
		}
...
/*ARGSUSED*/
void fgetarea (char *area, int32_t *count, char *dirname, int32_t arealen, int32_t dirlen)
{
    fgetarea_ (area, count, dirname, arealen, dirlen);
}


void area_list (char *area, int32_t *count, char *dirname)
{
    int32_t      arealen, dirlen;

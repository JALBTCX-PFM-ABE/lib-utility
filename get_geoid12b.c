...
/*  Check that the geoid12b files are available (returns 0 on success).  */

int8_t check_geoid12b ()
{
  int32_t        i;
  FILE           *fp;
  static char    dirfil[6][512], wvsdir[256];
  static char    file[6][20] = {"g2012bu0.bin", "g2012ba0.bin", "g2012bh0.bin", "g2012bp0.bin", "g2012bg0.bin", "g2012bs0.bin"};
...
/*!  Gets orthometric correction for specified lat/lon from geoid12b bin files.  */
  ...
  if (current_file != prev_file)
    {
      ...
      if (array == NULL)
        {
          perror ("Allocating geoid array memory in get_geoid12b");
          exit (-1);
        }
...
int32_t main (int32_t argc, char *argv[])
{
  ...
  if (argc < 2)
    {
      fprintf (stderr, "Usage: %s GEOID12B_FILE_NAME\n", argv[0]);
      fprintf (stderr, "Where GEOID12B_FILE_NAME is one of the following:\n");
      fprintf (stderr, "g2012bu0.bin, g2012ba0.bin, g2012bh0.bin, g2012bp0.bin, g2012bg0.bin, or g2012bs0.bin\n\n");
      exit (-1);
    }
